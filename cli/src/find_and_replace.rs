...
fn read_and_write(args: &Arguments){
    ...
    let replace_data = match replace(&args.patern, &args.replace, &data){
        Ok(d) => d,
        Err(e) => {
            eprintln!("{} failed to replace texxt: {:?}", "Error".red().bold(), e);
            std::process::exit(1);
        }
    };
...
fn parse_args() -> Arguments{
    ...
    if args.len() != 4 {
        print_help();
        eprintln!("{} wrong numebr of arguments. Expected 4, got {}", "Error".red().bold(), args.len());
        std::process::exit(1);
    }
